...
export default function AgentProgress({ steps }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 11, fontWeight: 700, color: C.sub, marginBottom: 12,
...
function StatusIcon({ status }: { status: AgentStep["status"] }) {
  if (status === "done")  return <span style={{ fontSize: 16, color: C.green, flexShrink: 0 }}>✓</span>;
  if (status === "error") return <span style={{ fontSize: 16, color: C.red,   flexShrink: 0 }}>✕</span>;
  if (status === "running") return (
    <span style={{
      width: 16, height: 16,
      border: `2px solid ${C.amberBd}`, borderTop: `2px solid ${C.amber}`,
      borderRadius: "50%", flexShrink: 0, display: "inline-block",
      animation: "spin 0.7s linear infinite",
    }} />
  );
  return <span style={{ width: 12, height: 12, borderRadius: "50%", border: `2px solid ${C.border}`, flexShrink: 0, display: "inline-block" }} />;
}
