...
const SYSTEM = `あなたはCVR（コンバージョン率）最適化の専門家です。
提供された情報をもとに、CTAやフォーム、コンバージョン導線の問題点を3〜7点、箇条書きで指摘してください。
各指摘は「【エリア名】具体的な問題点（改善案付き）」の形式で書いてください。
前置き・後書きは不要です。`;

export async function runCROAgent(input: {
  context:     string;
  ...
  if (fetchedLP) {
    ...
    parts.push(`CTA: ${fetchedLP.ctaTexts.slice(0, 8).join(" / ")}`);
  ...
  return { agentId: "cro", findings: findings.length > 0 ? findings : [text.trim()], priority: "high" };
}
