...
export async function runAgentLoop(input: OrchestratorInput): Promise<ReviewResult> {
  ...
  const step = (id: string, label: string, status: AgentStep["status"], message?: string) =>
  ...
  if (previousReview) {
    step("memory", "過去のレビューを確認", "done",
      `前回: ${previousReview.createdAt.slice(0, 10)}`);
  }
