...
export interface AICallInput {
  ...
  system:    string;
  userText:  string;
  imageB64s?: string[];   // base64 image data (without data: prefix)
  maxTokens?: number;
}
...
export async function callAI(input: AICallInput): Promise<string> {
  ...
  if (provider === "openai") {
    return callOpenAI({ modelId, apiKey: apiKey!, system, userText, imageB64s, maxTokens });
  }
  if (provider === "gemini") {
    return callGemini({ modelId, apiKey: apiKey!, system, userText, imageB64s, maxTokens });
  }
  // default: claude
  return callClaude({ modelId, apiKey, system, userText, imageB64s, maxTokens });
}
...
async function callClaude(input: {
  ...
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY ?? input.apiKey });
