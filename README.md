...
- **根拠のある指摘** — Research Agentが競合・業界ベストプラクティスを調査して根拠として提供
...
│   ├── page.tsx                      # メインUI・状態管理
│   └── api/
│       └── review/route.ts           # SSEエントリポイント → Orchestrator
├── components/
│   ├── AgentProgress.tsx             # エージェント進捗UI（リアルタイム）
...
| `BRAVE_SEARCH_API_KEY` | 任意 | Research Agentの競合調査に使用。未設定の場合はスキップして続行 |
